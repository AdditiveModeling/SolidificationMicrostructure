...
template <int dim, typename T> void update(grid<dim,vector<T> >& oldGrid, int steps)
{
	...
	const double     T_0 = 300.;  	// Temperature (isothermal simulation)
	...
	const double   dt = dx(oldGrid,0)*dx(oldGrid,0)/(1000.*D_L);
	...
	for (int step = 0; step < steps; step++) {
		...
		for (int i = 0; i < nodes(oldGrid); i++) {
			...
			double deltac = dc(x)[0]*lapC + graddc(x)[0]*gradc(x)[0] + graddc(x)[1]*gradc(x)[1] + temp*lapPhi;
			for (int k=0; k<dim; k++){
				deltac+=gradphi(x)[k]*gradtemp[k];
			}
			
				  double deltaphi = M_phi*((eb2*eta*eta*lapPhi - (1-oldX[1])*H_A - oldX[1]*H_B) + (eb2*eta*etap*(s2*(d2phi[0]) + 2*c2*phixy)) + (0.5*eb2*(etap*etap+eta*etapp)*(-2*s2*phixy+lapPhi+c2*(d2phi[0]))));
