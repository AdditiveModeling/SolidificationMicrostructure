// solidification.cpp
// Solidification by Warren1995 paper's method
// Based on the anisotropic solidification by 2D phase field method in MMSP
...
template <int dim, typename T> void update(grid<dim,vector<T> >& oldGrid, int steps)
{
	...
	const double     T_0 = 1574.;  	// Temperature (isothermal simulation)
	...
	const double   dt = dx(oldGrid,0)*dx(oldGrid,0)/(5.*D_L);
